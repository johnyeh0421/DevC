...
void hmac_md5(char *str, int str_len, unsigned char *key, int key_len, char rtn[33]);

int main(){
	
	char KEY[33];
	char MAC[] = "1234";
	#define	HASH_SECRET_KEY	 "test"
	hmac_md5(MAC, strlen(MAC), HASH_SECRET_KEY, strlen(HASH_SECRET_KEY), (char*)KEY);
	printf("MAC = %s\n", MAC);
	printf("HASH_SECRET_KEY = %s\n", HASH_SECRET_KEY);
	printf(">> KEY = %s\n\n\n", KEY);	
	system("pause");	
}
...
void hmac_md5(char *str, int str_len, unsigned char *key, int key_len, char rtn[33]){
	...
	if(key_len > 64){
		struct cvs_MD5Context tctx;
		cvs_MD5Init (&tctx);
		cvs_MD5Update (&tctx, (unsigned char const *)key, key_len);
		cvs_MD5Final (tk, &tctx);
		key = tk;
		key_len = 16;
	}
	...
	cvs_MD5Init (&context);
	cvs_MD5Update (&context, k_ipad, 64);
	cvs_MD5Update (&context, (unsigned char const *)str, str_len);
	cvs_MD5Final (checksum, &context);


	cvs_MD5Init (&context);
	cvs_MD5Update (&context, k_opad, 64);
	cvs_MD5Update (&context, checksum, 16);
	cvs_MD5Final (checksum, &context);
