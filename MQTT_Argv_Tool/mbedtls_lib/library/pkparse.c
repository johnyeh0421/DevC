...
{
#if 0
    if( strstr( path, CERTIFICATE_FILENAME) != NULL ){
	*buf = malloc(cps_cert_len);
	strcpy((char *)*buf , (const char *)cps_cert);
	*n = cps_cert_len;
    }
    else if( strstr( path, ROOT_CA_FILENAME) != NULL ){
	*buf = malloc(cps_rootca_len);
	strcpy((char *)*buf , (const char *)cps_rootca);
	*n = cps_rootca_len;
    }
    else if( strstr( path, PRIVATE_KEY_FILENAME) != NULL ){
	*buf = malloc(cps_private_key_len);
	strcpy((char *)*buf , (const char *)cps_private_key);
	*n = cps_private_key_len;
    }
    return( 0 );
