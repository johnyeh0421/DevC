...
extern char mqtt_host_name[128];
...
int iot_tls_connect(Network *pNetwork, TLSConnectParams params) {
	...
	mbedtls_pk_parse_keyfile(&pkey, params.pDevicePrivateKeyLocation, "");
	if (ret != 0) {
	...
	char port_string[6];
	sprintf(port_string, "%d", params.DestinationPort);
	printf("tls port_string = %s\n", port_string);
	if ((ret = mbedtls_net_connect(&server_fd, params.pDestinationURL, port_string, MBEDTLS_NET_PROTO_TCP)) != 0) {
	...
	DEBUG("End of iot_tls_connect return ret\n");
	return ret;
}
